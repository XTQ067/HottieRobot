...
async def gben(ubot):
    ...
    if ubot.is_private:
        user = ubot.chat
        rank = ubot.pattern_match.group(1)
    else:
        ubot.chat.title
    try:
        user, rank = await get_full_user(ubot)
    except:
        pass
    if me == user:
    ...
    if user:
        telchanel = [d.entity.id
                     for d in await ubot.client.get_dialogs()
        ...
        for x in telchanel:
          try:
             await ubot.client(EditAdminRequest(x, user, rgt, rank))
             i += 1
